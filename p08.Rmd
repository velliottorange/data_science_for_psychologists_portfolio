...
ca_final <- ca_raw %>%
  ...
  # 2. Clean citation noise (ex. [1])
  mutate(across(where(is.character), ~str_remove_all(., "\\[\\d+\\]"))) %>%
  
  # 3. Standardize the status column
  mutate(
  ...
  # 4. Extract numeric data and drop originals
  mutate(
  ...
  # 5. Final cleanup and map-readiness
  select(-founded, -abandoned, -latitude_longitude) %>%

  # 6. Remove "Out of Bounds" points
...
# ---- 2. Run the histogram on the new plotting_data ----
